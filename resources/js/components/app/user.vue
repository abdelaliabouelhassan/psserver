<template>
  <div>
          ...
          <tr class="py-5" v-for="(myserve, index) in myservers">
            ...
            <td>
              <img src="img/like.png" class="img-fluid" alt="" />{{
                myserve.realtimeVote
              }}
            </td>
            <td>
              <i class="fa fa-external-link-alt text-dark font-10 mr-1"></i
              >{{ myserve.viewd }}
            </td>
            <td class="text-center">
              <button
                class="btn btn-dark px-3 py-0 rounded m-0"
                style="font-size: 12px"
                @click="editServer(myserve.slug)"
              >
                {{ $t("message.edit") }}
              </button>
              <button
                ...
                "
                @click="deactive(myserve.id)"
                class="btn btn-danger px-3 py-0 rounded m-0"
                style="font-size: 12px"
              >
              ...
              <button
                v-if="
                  ...
                  !myserve.server_owner_active
                "
                @click="active(myserve.id)"
                class="btn btn-success px-3 py-0 rounded m-0"
                style="font-size: 12px"
              >
                {{ $t("message.Active") }}
              </button>
    ...
    <button
      class="btn btn-dark bg-dark btn-block rounded text-white font-14 mt-5"
    >
      ...
      <div class="col-md-6">
        <div action="">
          ...
          <div class="text-center">
            <input
              type="submit"
              class="btn btn-danger rounded font-14 mt-4"
              @click="changepassword"
              :disabled="clicked"
              :value="$t('message.CHANGE_PASSWORD')"
            />
          </div>
        </div>
      </div>
